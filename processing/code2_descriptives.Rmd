...
# Preferencias redistributivas por año y país
...
figure1<-ggplot(redistribution_avg_country, aes(x = reorder(country, promedio_redistribucion), y = promedio_redistribucion)) +
  geom_point(size = 2.5, color = "black") +  # Puntos más grandes y color ajustado
...
# Crear un gráfico de líneas con etiquetas alineadas al final de cada línea
figure2<-ggplot(redistribution_avg, aes(x = year, y = promedio_redistribucion, group = country)) +
  geom_line(aes(linetype = country), size = 1, na.rm = TRUE) +  # Diferenciar por tipo de línea
  scale_linetype_manual(values = line_types) +  # Asignar manualmente los tipos de línea
  geom_label_repel(data = redistribution_avg %>% 
                     group_by(country) %>% 
                   ...
                   segment.size = 0.5, 
                   force = 2, 
                   max.overlaps = 15) +  # Etiquetas de los países con líneas que conectan
  labs(x = "Año",
       y = "Media de Preferencias Redistributivas") +
  theme_minimal() +
  theme(legend.position = "none") +  # Eliminar la leyenda
  scale_x_continuous(breaks = seq(min(redistribution_avg$year), max(redistribution_avg$year), by = 2), 
                     expand = expansion(mult = c(0.05, 0.1)))  # Expandir el espacio en el eje X para las etiquetas
...
```{r Figure 2, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
figure4
ggsave(here("output", "graphs", "Figure4.jpg"), plot=figure4, height = 4, units="in")
...
figure5
ggsave(here("output", "graphs", "Figure5.jpg"), plot=figure5, height = 4, units="in")
...
figure6
ggsave(here("output", "graphs", "Figure6.jpg"), plot=figure6, height = 4, units="in")
```
...
figure7
ggsave(here("output", "graphs", "Figure7.jpg"), plot=figure7, height = 4, units="in")
```
...
figure10
ggsave(here("output", "graphs", "Figure10.jpg"), plot=figure10, height = 4, units="in")
```
